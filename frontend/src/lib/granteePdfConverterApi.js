...
/**
 * POST `pdf` to backend `/upload`, read returned .xlsx, map rows to grantee import shape.
 * @param {File} file
 * @returns {Promise<Record<string, string>[]>}
 */
export async function parseGranteePdfViaBackend(file) {
  const base = getPdfConverterBaseUrl()
  const form = new FormData()
  form.append("pdf", file, file.name)
  ...
  return json.map((row) => normalizeBackendImportRow(row)).filter((row) => Object.keys(row).length > 0)
}
