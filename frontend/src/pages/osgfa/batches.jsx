...
import { CalendarDays, GraduationCap, Layers, Plus, Search, SlidersHorizontal, TableProperties } from "lucide-react"
...
export default function Batches() {
  ...
  // Fetch all records from backend database server on layout mount
  useEffect(() => {
    const fetchLiveGrantees = async () => {
      try {
        setIsLoading(true)
        const response = await fetch("http://localhost:5000/api/grantees")
        if (!response.ok) {
          throw new Error("Failed to pull live grantee collection rows from API endpoint.")
        }
        const data = await response.json()
        setGranteesRawData(Array.isArray(data) ? data : [])
      } catch (err) {
  ...
  // Aggregate single items into a unique batches card descriptor array
  ...
  return (
    <section className="w-full min-w-0 max-w-full space-y-4">
      ...
      {/* Grid Content Data State Output */}
      {isLoading ? (
      ...
      {/* Floating Plus Context Form Action Trigger */}
      <button
        type="button"
