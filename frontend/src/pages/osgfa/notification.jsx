...
export default function NotificationPage() {
  ...
  useEffect(() => {
    ...
    window.addEventListener(OSGFA_SETTINGS_CHANGED_EVENT, syncPrefs)
    window.addEventListener("storage", syncPrefs)
    return () => {
      window.removeEventListener(OSGFA_SETTINGS_CHANGED_EVENT, syncPrefs)
      window.removeEventListener("storage", syncPrefs)
    }
  }, [])

  const visibleNotifications = useMemo(
    () => notifications.filter((item) => shouldShowNotification(item, notificationPrefs)),
  ...
  const hiddenByPreferencesCount = notifications.length - visibleNotifications.length
  ...
  const selectedNotification = useMemo(
    () => visibleNotifications.find((item) => item.id === selectedNotificationId) ?? null,
    [visibleNotifications, selectedNotificationId],
  )
    ...
    <section className="w-full min-w-0 max-w-full space-y-4">
      ...
      {hiddenByPreferencesCount > 0 ? (
        <p className="text-xs text-slate-500">
          {hiddenByPreferencesCount} notification{hiddenByPreferencesCount === 1 ? "" : "s"} hidden by your
